...
import { Sprout, Loader2, AlertCircle, Key } from "lucide-react";
...
export default function SetupAccess() {
  ...
  const onAuth = async (data: AuthFormValues) => {
    ...
    try {
      ...
      // Mark link as used
      if (linkCode) {
        await fetch(`/api/access-links/${linkCode}/use`, { method: 'POST' });
      }
      ...
      <div className="w-full flex items-center justify-center p-4 relative z-10">
        <Card className="w-full max-w-md shadow-2xl border-none bg-white/95 backdrop-blur-xl">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardDescription>Use este link para criar sua conta e senha.</CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={authForm.handleSubmit(onAuth)} className="space-y-4">
              ...
              <Button type="submit" className="w-full" size="lg" disabled={isLoading}>
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Configurar Acesso"}
              </Button>
            </form>
          </CardContent>
        </Card>
